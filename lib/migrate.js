...
const runSqlFile = function(directoryName, filename, pool){
...
const addCompletedMigration = function(filename, pool){
    return new Promise(function(resolve, reject){
...
const runMigration = async function(directoryName, filename, pool){
    console.log(`Migration ${filename} has started`);
    const {name, ext} = path.parse(filename);
    if(ext === '.sql'){
        await runSqlFile(directoryName, filename, pool);
    }
    else{
...
const runMigrations = async function(directoryName, filenames, pool){
    await forEachAsync(filenames, async function(file){
        let filename;
        try {
            filename = file.m;
            await runMigration(directoryName, filename, pool);
            await addCompletedMigration(filename, pool);
        }
        catch (e){
...
const getCurrentMigrations = function(sql, pool){
    return new Promise(function(resolve, reject){
        pool.query(sql, function(err, results){
            if(err){
                console.log('err: ', err);
...
const run = async function(directoryName, pool, clear){
...
(
    ...
    const filenames = await getCurrentMigrations(sql, pool);
    if (!filenames.length){
        console.log('done');
        return;
    }
    ...
    return runMigrations(directoryName, filenames, pool);
};
