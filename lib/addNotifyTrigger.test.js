...
const addNotifyTrigger = require('./addNotifyTrigger');
...
describe('addNotifyTrigger({tableName, triggerName, triggersPath})', () => {
    ...
    test('expects to default to `argument.triggerName` instead `argument.tableName` if `argument.triggerName` exists', () => {
        for(let i = 0; i < 100; i++){
            argument.tableName += 'a';
        }
        const x = addNotifyTrigger(argument);
